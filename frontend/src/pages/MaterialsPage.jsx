...
export default function MaterialsPage() {
  ...
  const { data: materialsData, isLoading, error } = useMaterials(id)
  const uploadMutation = useUploadMaterial(id)
  ...
  function uploadFile(file) {
    ...
    uploadMutation.mutate({ file, onProgress: setUploadProgress }, {
      onSuccess: () => { addToast('File uploaded successfully!', 'success'); setUploadProgress(0) },
      onError: (err) => { addToast(err.message, 'error'); setUploadProgress(0) },
    })
  }
    ...
    <div className="min-h-screen bg-bg">
      ...
      <ConfirmModal
        ...
        onConfirm={() => deleteMutation.mutate(deleteTarget?.id, {
          onSuccess: () => { addToast('Material deleted.', 'info'); setDeleteTarget(null) },
