    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard
          icon={Star}
          label="Avg Rating"
          value={stats.average_rating != null ? Number(stats.average_rating).toFixed(1) : '0.0'}
          loading={summaryLoading}
        />
        <StatCard
          icon={TrendingUp}
...
function EventsReportTab() {
  ...
  const { data, isLoading } = useEventsReport(month, year)
              ...
              <tbody>
                {events.map((ev, idx) => (
                  <tr key={ev.id ?? idx} className="border-b border-border hover:bg-surface-alt/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right text-fg-2">{ev.average_rating != null ? Number(ev.average_rating).toFixed(1) : '—'}</td>
                    <td className="px-4 py-3">
...
function OrganizersReportTab() {
  ...
  const { data, isLoading } = useOrganizersReport()
              ...
              <thead>
                <tr className="border-b border-border bg-surface-alt">
                  {[
                    ...
                    { key: 'last_event_date', label: 'Last Event' },
                  ].map(({ key, label }) => (
                    <th
                      key={key}
              ...
              <tbody>
                {filtered.map((org, idx) => (
                  <tr key={org.id ?? idx} className="border-b border-border hover:bg-surface-alt/50 transition-colors">
                    <td className="px-4 py-3 text-fg font-medium">{org.full_name || '—'}</td>
                    ...
                    <td className="px-4 py-3 text-fg-2 text-right">{org.average_rating != null ? Number(org.average_rating).toFixed(1) : '—'}</td>
                    <td className="px-4 py-3 text-fg-3 whitespace-nowrap">{formatDate(org.last_event_date)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
