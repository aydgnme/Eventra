...
_SENTINEL_IDS = count(1_000_000, 10_000)
...
def _seed_sqlite_identity():
    sentinel_id = next(_SENTINEL_IDS)
    sentinel = Event(
        id=sentinel_id,
        title="Test identity sentinel",
        start_datetime=datetime.now(timezone.utc) + timedelta(days=3650),
        end_datetime=datetime.now(timezone.utc) + timedelta(days=3650, hours=1),
        organizer_id=-1,
        is_published=False,
    )
...
@pytest.fixture
def published_event(client, organizer_headers, app):
    resp = client.post("/events/", json={**BASE_EVENT, "is_published": True}, headers=organizer_headers)
    assert resp.status_code == 201, resp.get_json()
    return publish_event(app, resp.get_json()["event"]["id"])
...
def publish_event(app, event_id):
    with app.app_context():
