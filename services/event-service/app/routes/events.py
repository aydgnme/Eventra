...
def my_events():
    ...
    for e in events:
        ...
        d["review_status"] = review.status if review else "pending"
        d["rejection_reason"] = review.rejection_reason if review else None
        result.append(d)
...
def event_qr(event_id):
    """Generate a QR code PNG for the event URL."""
    event, err = _get_event_or_404(event_id)
    if err:
        return err
    ...
    try:
        ...
        return jsonify({"error": "QR generation not available"}), 501
    ...
    return send_file(
        buf,
        mimetype="image/png",
        download_name=f"event-{event_id}-qr.png",
    )
...
def event_ics(event_id):
    ...
    try:
        ...
        return jsonify({"error": "ICS export not available"}), 501

    cal = Calendar()
    cal.add("prodid", "-//Eventra//Event Export//EN")
    cal.add("version", "2.0")
    cal.add("calscale", "GREGORIAN")

    cal_event = CalEvent()
    cal_event.add("uid", f"event-{event_id}@eventra")
    ...
    return send_file(
        ...
        as_attachment=True,
        download_name=f"{event.title.replace(' ', '-').lower()}.ics",
    )
